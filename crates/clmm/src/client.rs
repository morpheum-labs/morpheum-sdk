...
use crate::requests;
use crate::types::{
    BoostedBuffer, ClmmPosition, GlideSimulation, LiquidityDepthBand, PoolFeeStats,
...
fn check_success(success: bool, err: &str) -> Result<(), SdkError> {
    if success {
        ...
        Err(SdkError::transport(if err.is_empty() {
            "operation failed"
        } else {
            err
        }))
...
impl ClmmClient {
    ...
    pub async fn get_quote(
        &self,
        request: requests::GetQuoteRequest,
    ) -> Result<QuoteResult, SdkError> {
        let proto_req: proto::GetQuoteRequest = request.into();
        let resp = self
            .query("/clmm.v1.Query/GetQuote", proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn get_liquidity_depth(
        &self,
        request: requests::GetLiquidityDepthRequest,
    ) -> Result<Vec<LiquidityDepthBand>, SdkError> {
        let proto_req: proto::GetLiquidityDepthRequest = request.into();
        let resp = self
            .query(
                "/clmm.v1.Query/GetLiquidityDepth",
                proto_req.encode_to_vec(),
            )
            .await?;
        let p =
            proto::GetLiquidityDepthResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        check_success(p.success, &p.error_message)?;
    ...
    pub async fn get_pool_risk_summary(
        &self,
        pool_id: impl Into<alloc::string::String>,
        ...
        let resp = self
            ...
            .await?;
        let p =
            proto::GetPoolRiskSummaryResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        check_success(p.success, &p.error_message)?;
        Ok(PoolRiskSummary {
            pool_id: p.pool_id,
    ...
    pub async fn get_boosted_buffer(
        ...
        let resp = self
            .query("/clmm.v1.Query/GetBoostedBuffer", proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetBoostedBufferResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        check_success(p.success, &p.error_message)?;
        Ok(BoostedBuffer {
            pool_id: p.pool_id,
            buffer_amount: p.buffer_amount,
            pending_yield: p.pending_yield,
            apy_estimate: p.apy_estimate,
        })
    }
    ...
    pub async fn query_pool_fee_stats(
        ...
        let resp = self
            .query(
                "/clmm.v1.Query/QueryPoolFeeStats",
                proto_req.encode_to_vec(),
            )
            .await?;
        let p =
            proto::QueryPoolFeeStatsResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
    ...
    pub async fn simulate_reclmm_glide(
        &self,
        request: requests::SimulateReClmmGlideRequest,
    ) -> Result<GlideSimulation, SdkError> {
        let proto_req: proto::SimulateReClmmGlideRequest = request.into();
        let resp = self
            .query(
                "/clmm.v1.Query/SimulateReClmmGlide",
                proto_req.encode_to_vec(),
            )
            .await?;
        let p = proto::SimulateReClmmGlideResponse::decode(resp.as_slice())
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                ...
                "/clmm.v1.Query/SimulateSwap" => Ok(prost::Message::encode_to_vec(
                    &proto::SimulateSwapResponse {
                        success: true,
                        amount_in: "1000".into(),
                        amount_out: "990".into(),
                        fee_amount: "3".into(),
                        price_impact: "0.1".into(),
                        error_message: String::new(),
                        timestamp: None,
                    },
                )),
                "/clmm.v1.Query/GetPoolRiskSummary" => Ok(prost::Message::encode_to_vec(
                    &proto::GetPoolRiskSummaryResponse {
                        success: true,
                        pool_id: "0x1234".into(),
                        health_score_bps: "9500".into(),
                        utilization_rate: "0.45".into(),
                        concentration_risk: "0.2".into(),
                        error_message: String::new(),
                        timestamp: None,
                    },
                )),
    ...
    async fn simulate_swap_works() {
        let sim = make_client()
            .simulate_swap(requests::SimulateSwapRequest::new(
                "0x1234",
                42,
                Side::Buy,
                "1000",
