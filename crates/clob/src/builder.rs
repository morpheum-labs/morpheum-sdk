...
impl PlaceOrderBuilder {
    ...
    pub fn market_index(mut self, i: u64) -> Self {
        self.market_index = Some(i);
        self
    }
    pub fn price(mut self, p: impl Into<String>) -> Self {
        self.price = Some(p.into());
        self
    }
    pub fn quantity(mut self, q: impl Into<String>) -> Self {
        self.quantity = Some(q.into());
        self
    }
    ...
    pub fn order_type(mut self, t: OrderType) -> Self {
        self.order_type = Some(t);
        self
    }
    ...
    pub fn leverage(mut self, l: impl Into<String>) -> Self {
        self.leverage = Some(l.into());
        self
    }
    ...
    pub fn build(self) -> Result<PlaceOrderRequest, SdkError> {
        ...
        let price = self
            .price
            .ok_or_else(|| SdkError::invalid_input("price is required"))?;
        let quantity = self
            .quantity
            .ok_or_else(|| SdkError::invalid_input("quantity is required"))?;
        let side = self
            .side
            .ok_or_else(|| SdkError::invalid_input("side is required"))?;
        let order_type = self
            .order_type
            .ok_or_else(|| SdkError::invalid_input("order_type is required"))?;

        let mut req =
            PlaceOrderRequest::new(address, market_index, price, quantity, side, order_type);
        if let Some(v) = self.client_order_id {
            req = req.client_order_id(v);
        }
        if let Some(v) = self.leverage {
            req = req.leverage(v);
        }
        if let Some(v) = self.take_profit {
            req = req.take_profit(v);
        }
        if let Some(v) = self.stop_loss {
            req = req.stop_loss(v);
        }
        if let Some(v) = self.time_in_force {
            req = req.time_in_force(v);
        }
        if self.post_only {
            req = req.post_only(true);
        }
        if self.hidden {
            req = req.hidden(true);
        }
        if self.reduce_only {
            req = req.reduce_only(true);
        }
...
impl ModifyOrderBuilder {
    ...
    pub fn build(self) -> Result<ModifyOrderRequest, SdkError> {
        ...
        let order_id = self
            .order_id
            .ok_or_else(|| SdkError::invalid_input("order_id is required"))?;
        let mut req = ModifyOrderRequest::new(address, order_id);
        if let Some(v) = self.symbol {
            req = req.symbol(v);
        }
        if let Some(v) = self.new_price {
            req = req.new_price(v);
        }
        if let Some(v) = self.new_quantity {
            req = req.new_quantity(v);
        }
        Ok(req)
    }
}
...
impl ProvideMarketMakerQuoteBuilder {
    ...
    pub fn build(self) -> Result<ProvideMarketMakerQuoteRequest, SdkError> {
        let provider = self
            .provider
            .ok_or_else(|| SdkError::invalid_input("provider is required"))?;
        let pool_id = self
            .pool_id
        ...
        Ok(ProvideMarketMakerQuoteRequest::new(
            provider,
            pool_id,
            market_index,
            side,
...
impl CancelMarketMakerQuoteBuilder {
    ...
    pub fn build(self) -> Result<CancelMarketMakerQuoteRequest, SdkError> {
        let quote_id = self
            .quote_id
            .ok_or_else(|| SdkError::invalid_input("quote_id is required"))?;
        let provider = self
            .provider
            .ok_or_else(|| SdkError::invalid_input("provider is required"))?;
