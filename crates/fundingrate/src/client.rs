...
impl FundingRateClient {
    ...
    pub async fn get_funding_rate(
        &self,
        market_index: u64,
    ) -> Result<FundingRateSnapshot, SdkError> {
    ...
    pub async fn get_next_funding_time(&self, market_index: u64) -> Result<u64, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p =
            proto::GetNextFundingTimeResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        Ok(p.next_funding_time)
    }

    /// Gets the funding market profile for a market.
    pub async fn get_market_profile(
        &self,
        market_index: u64,
    ) -> Result<FundingMarketProfile, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p =
            proto::GetMarketProfileResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.profile
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("profile field missing"))
    }
}
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                "/fundingrate.v1.Query/GetFundingRate" => Ok(prost::Message::encode_to_vec(
                    &proto::GetFundingRateResponse {
                        funding_rate: -500,
                ...
                "/fundingrate.v1.Query/GetNextFundingTime" => Ok(prost::Message::encode_to_vec(
                    &proto::GetNextFundingTimeResponse {
                        next_funding_time: 1_700_028_800,
                    },
                )),
                "/fundingrate.v1.Query/GetMarketProfile" => Ok(prost::Message::encode_to_vec(
                    &proto::GetMarketProfileResponse {
    ...
    async fn get_market_profile_works() {
        ...
        assert_eq!(
            profile.mode,
            crate::types::FundingApplicationMode::BothSides
        );
    }
}
