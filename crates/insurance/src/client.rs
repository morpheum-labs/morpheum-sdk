...
impl InsuranceClient {
    ...
    ) -> Result<VaultBalance, SdkError> {
        let req = requests::GetVaultBalanceRequest::new(asset_index);
        let proto_req: proto::GetVaultBalanceRequest = req.into();
        let resp = self
            .query(svc_path!("GetVaultBalance"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetVaultBalanceResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.balance
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("balance field missing"))
    }

    /// Gets a specific LP stake by address.
    pub async fn get_lp_stake(
        &self,
        req: requests::GetLpStakeRequest,
    ) -> Result<LpStake, SdkError> {
        let proto_req: proto::GetLpStakeRequest = req.into();
        let resp = self
            .query(svc_path!("GetLPStake"), proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn list_lp_stakes(
        &self,
        req: requests::ListLpStakesRequest,
    ) -> Result<(Vec<LpStake>, PageInfo), SdkError> {
        let proto_req: proto::ListLpStakesRequest = req.into();
        let resp = self
            .query(svc_path!("ListLPStakes"), proto_req.encode_to_vec())
            .await?;
        ...
        let page = p.pagination.map_or(
            PageInfo {
                next_key: Vec::new(),
                total: 0,
            },
            |pg| PageInfo {
                next_key: pg.next_key,
                total: pg.total,
            },
        );
        Ok((stakes, page))
    ...
    pub async fn get_bad_debt_history(
        &self,
        req: requests::GetBadDebtHistoryRequest,
    ) -> Result<(Vec<BadDebtRecord>, PageInfo), SdkError> {
        let proto_req: proto::GetBadDebtHistoryRequest = req.into();
        let resp = self
            .query(svc_path!("GetBadDebtHistory"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetBadDebtHistoryResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
    ...
    pub async fn get_threshold_status(
        &self,
        asset_index: Option<u64>,
    ) -> Result<ThresholdStatus, SdkError> {
        let req = requests::GetThresholdStatusRequest::new(asset_index);
        let proto_req: proto::GetThresholdStatusRequest = req.into();
        let resp = self
            .query(svc_path!("GetThresholdStatus"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetThresholdStatusResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.status
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("status field missing"))
    }
}
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                "/insurance.v1.InsuranceService/GetVaultBalance" => Ok(
                    prost::Message::encode_to_vec(&proto::GetVaultBalanceResponse {
                        success: true,
                ...
                "/insurance.v1.InsuranceService/ListLPStakes" => Ok(prost::Message::encode_to_vec(
                    &proto::ListLpStakesResponse {
                        success: true,
                        stakes: vec![],
                        pagination: None,
                    },
                )),
                "/insurance.v1.InsuranceService/GetThresholdStatus" => Ok(
                    prost::Message::encode_to_vec(&proto::GetThresholdStatusResponse {
                        success: true,
                        status: Some(proto::ThresholdStatus {
                            asset: None,
                            near_depletion: false,
                            paused: false,
                            depletion_percentage: "5000".into(),
                            last_checked: None,
                        }),
                    }),
                ),
