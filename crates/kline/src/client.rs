...
impl KlineClient {
    ...
    pub async fn get_mark_price_with_spread(
        ...
        let resp = self
            ...
            .await?;
        let p = proto::GetMarkPriceWithSpreadResponse::decode(resp.as_slice())
    ...
    pub async fn get_vwap(
        ...
        let resp = self
            .query("/kline.v1.Query/GetVWAP", proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn get_long_short_ratio(
        ...
        let resp = self
            ...
            .await?;
        let p =
            proto::GetLongShortRatioResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        Ok(LongShortRatio {
            ratio: p.ratio,
            long_oi: p.long_oi,
            short_oi: p.short_oi,
    ...
    pub async fn get_last_kline(
        ...
        let resp = self
            .query("/kline.v1.Query/GetLastKline", proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn query_klines_snapshot(
        ...
        let req =
            requests::QueryKlinesSnapshotRequest::new(market_index, interval, start_time, end_time);
        ...
        let resp = self
            ...
            .await?;
        let p = proto::QueryKlinesSnapshotResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                "/kline.v1.Query/GetMarkPriceWithSpread" => Ok(prost::Message::encode_to_vec(
                    &proto::GetMarkPriceWithSpreadResponse {
                        mark: 50_000_000_000,
                        spread: 100_000_000,
                        lower_band: 49_900_000_000,
                        upper_band: 50_100_000_000,
                        quality: 95_000_000,
                        proof: vec![0u8; 32],
                    },
                )),
                ...
                "/kline.v1.Query/GetLongShortRatio" => Ok(prost::Message::encode_to_vec(
                    &proto::GetLongShortRatioResponse {
                        ratio: 150_000_000,
                        long_oi: "600".into(),
                        short_oi: "400".into(),
                        proof: vec![],
                ...
                "/kline.v1.Query/GetLastKline" => Ok(prost::Message::encode_to_vec(
                    &proto::GetLastKlineResponse {
                        open: 49_500,
                        high: 51_000,
                        low: 49_000,
                        close: 50_500,
                        volume_base: "500".into(),
                        proof: vec![],
                    },
                )),
                "/kline.v1.Query/QueryKlinesSnapshot" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryKlinesSnapshotResponse {
                        success: true,
                        klines: vec![],
                    },
                )),
    ...
    async fn get_mark_price_with_spread_works() {
        let mp = make_client()
            .get_mark_price_with_spread(1, 100)
            .await
            .unwrap();
    ...
    async fn query_klines_snapshot_works() {
        let klines = make_client()
            .query_klines_snapshot(1, "1h", 100, 200)
            .await
            .unwrap();
