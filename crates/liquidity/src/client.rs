...
macro_rules! svc_path {
    ($method:expr) => {
        concat!("/liquidity.v1.LiquidityService/", $method)
    };
}
...
impl LiquidityClient {
    ...
    pub async fn get_pool(&self, pool_id: &str) -> Result<Pool, SdkError> {
        ...
        let resp = self
            .query(svc_path!("GetPool"), proto_req.encode_to_vec())
            .await?;
        let p = proto::GetPoolResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.pool
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("pool field missing"))
    }

    /// Lists all pools with pagination and optional status filter.
    pub async fn list_pools(
        &self,
        req: requests::ListPoolsRequest,
    ) -> Result<(Vec<Pool>, PageInfo), SdkError> {
        let proto_req: proto::ListPoolsRequest = req.into();
        let resp = self
            .query(svc_path!("ListPools"), proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn get_pools_by_market(
        &self,
        req: requests::GetPoolsByMarketRequest,
    ) -> Result<(Vec<Pool>, PageInfo), SdkError> {
        let proto_req: proto::GetPoolsByMarketRequest = req.into();
        let resp = self
            .query(svc_path!("GetPoolsByMarket"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetPoolsByMarketResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        let pools = p.pools.into_iter().map(Into::into).collect();
        let page = p.pagination.map_or(
            PageInfo {
                next_key: Vec::new(),
    ...
    pub async fn get_depth_metrics(&self, market_index: u64) -> Result<DepthMetrics, SdkError> {
        ...
        let resp = self
            .query(svc_path!("GetDepthMetrics"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetDepthMetricsResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.metrics
            .map(Into::into)
    ...
    pub async fn get_pool_health(&self, pool_id: &str) -> Result<PoolHealth, SdkError> {
        ...
        let resp = self
            .query(svc_path!("GetPoolHealth"), proto_req.encode_to_vec())
            .await?;
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                ...
                "/liquidity.v1.LiquidityService/GetDepthMetrics" => Ok(
                    prost::Message::encode_to_vec(&proto::GetDepthMetricsResponse {
                        success: true,
                ...
                "/liquidity.v1.LiquidityService/GetPoolHealth" => Ok(
                    prost::Message::encode_to_vec(&proto::GetPoolHealthResponse {
                        success: true,
                        health: Some(proto::PoolHealthSnapshot {
                            pool_id: "pool1".into(),
                            health_score_bps: 9500,
                            utilization_rate: "6500".into(),
                            apy: "1200".into(),
                            timestamp: None,
                        }),
                    }),
    ...
    async fn list_pools_works() {
        let (pools, page) = make_client()
            .list_pools(requests::ListPoolsRequest::new(0, 50))
            .await
            .unwrap();
