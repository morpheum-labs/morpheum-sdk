...
impl MarkPriceClient {
    ...
    /// Returns `None` if no price is available.
    pub async fn get_mark_price_with_source(
        &self,
        market_index: u64,
    ) -> Result<Option<MarkPriceWithSource>, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::GetMarkPriceWithSourceResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
        Ok(if p.found {
            Some(MarkPriceWithSource {
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                "/markprice.v1.Query/GetMarkPrice" => Ok(prost::Message::encode_to_vec(
                    &proto::GetMarkPriceResponse {
                        found: true,
                        mark_price: 50_000_000_000,
                    },
                )),
                "/markprice.v1.Query/GetMarkPriceWithSource" => Ok(prost::Message::encode_to_vec(
                    &proto::GetMarkPriceWithSourceResponse {
                        found: true,
                        mark_price: 50_000_000_000,
                        source: 1,
                    },
