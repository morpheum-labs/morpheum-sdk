...
impl OsaClient {
    ...
    pub async fn get_account(
        &self,
        account_id: &str,
    ) -> Result<OutcomeSettlementAccount, SdkError> {
        let req = requests::GetAccountRequest::new(account_id);
        let proto_req: proto::QueryGetAccountRequest = req.into();
        let resp = self
            .query("/osa.v1.Query/GetAccount", proto_req.encode_to_vec())
            .await?;
        let p =
            proto::QueryGetAccountResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.account
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("account field missing"))
    }
    ...
    pub async fn get_balance(&self, account_id: &str, address: &str) -> Result<Balance, SdkError> {
        ...
        let resp = self
            .query("/osa.v1.Query/GetBalance", proto_req.encode_to_vec())
            .await?;
        let p =
            proto::QueryGetBalanceResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.balance
            .map(Into::into)
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                "/osa.v1.Query/GetAccount" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryGetAccountResponse {
                        success: true,
                ...
                "/osa.v1.Query/GetBalance" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryGetBalanceResponse {
                        success: true,
                        error_message: String::new(),
                        balance: Some(proto::Balance {
                            shares: 500,
