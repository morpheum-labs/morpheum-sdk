...
impl OutcomeFeedClient {
    ...
    pub async fn get_resolved_outcome(&self, feed_id: &str) -> Result<ResolvedOutcome, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::QueryResolvedOutcomeResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
        p.outcome
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("outcome field missing"))
    }

    /// Gets a single prediction market feed by ID.
    pub async fn get_prediction_feed(
        &self,
        feed_id: &str,
    ) -> Result<PredictionMarketFeed, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::QueryPredictionFeedResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
        p.feed
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("feed field missing"))
    }

    /// Lists prediction market feeds with pagination and optional filters.
    pub async fn list_prediction_feeds(
        &self,
        req: requests::QueryPredictionFeedsRequest,
    ) -> Result<PredictionFeedsPage, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::QueryPredictionFeedsResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                "/outcomefeed.v1.Query/QueryResolvedOutcome" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryResolvedOutcomeResponse {
                        success: true,
                ...
                "/outcomefeed.v1.Query/QueryPredictionFeed" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryPredictionFeedResponse {
                        success: true,
                        error_message: String::new(),
                ...
                "/outcomefeed.v1.Query/QueryPredictionFeeds" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryPredictionFeedsResponse {
                        success: true,
                        error_message: String::new(),
                        feeds: alloc::vec![],
                        total_count: 0,
