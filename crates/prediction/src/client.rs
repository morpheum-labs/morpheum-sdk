...
impl PredictionClient {
    ...
    pub async fn get_market(&self, feed_id: &str) -> Result<PredictionMarket, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::QueryPredictionMarketResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
        p.market
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("market field missing"))
    }

    /// Lists prediction markets with pagination and optional phase filter.
    pub async fn list_markets(
        &self,
        req: requests::QueryPredictionMarketsRequest,
    ) -> Result<PredictionMarketsPage, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::QueryPredictionMarketsResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
    ...
    pub async fn get_implied_probability(&self, feed_id: &str) -> Result<u32, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::QueryPredictionImpliedProbabilityResponse::decode(resp.as_slice())
            .map_err(SdkError::Decode)?;
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                "/prediction.v1.Query/QueryPredictionMarket" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryPredictionMarketResponse {
                        success: true,
                ...
                "/prediction.v1.Query/QueryPredictionMarkets" => Ok(prost::Message::encode_to_vec(
                    &proto::QueryPredictionMarketsResponse {
                        success: true,
                        error_message: String::new(),
                        markets: vec![],
                        total_count: 0,
                ...
                "/prediction.v1.Query/QueryPredictionImpliedProbability" => {
                    Ok(prost::Message::encode_to_vec(
                        &proto::QueryPredictionImpliedProbabilityResponse {
                            success: true,
                            error_message: String::new(),
