...
impl PriceFeedClient {
    ...
    pub async fn get_feed(&self, feed_index: u64) -> Result<PriceFeed, SdkError> {
        ...
        let resp = self
            .query("/pricefeed.v1.Query/QueryFeed", proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn get_feed_by_symbol(&self, symbol: &str) -> Result<PriceFeed, SdkError> {
        ...
        let resp = self
            .query(
                "/pricefeed.v1.Query/QueryFeedBySymbol",
                proto_req.encode_to_vec(),
            )
            .await?;
        let p =
            proto::QueryFeedBySymbolResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.feed
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("feed field missing"))
    }
    ...
    pub async fn list_feeds(
        &self,
        req: requests::QueryFeedsRequest,
    ) -> Result<FeedsPage, SdkError> {
        let proto_req: proto::QueryFeedsRequest = req.into();
        let resp = self
            .query("/pricefeed.v1.Query/QueryFeeds", proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn get_price(&self, feed_index: u64) -> Result<PriceEntry, SdkError> {
        ...
        let resp = self
            .query("/pricefeed.v1.Query/QueryPrice", proto_req.encode_to_vec())
            .await?;
        let p = proto::QueryPriceResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.price
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("price field missing"))
    }
}
