...
impl RiskClient {
    ...
    pub async fn get_oi_ratio(&self, market_index: u64) -> Result<OiRatioResult, SdkError> {
        ...
        let resp = self
            .query("/risk.v1.Query/GetOiRatio", proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn get_maintenance_margin(
        &self,
        req: requests::GetMaintenanceMarginRequest,
    ) -> Result<alloc::string::String, SdkError> {
        ...
        let resp = self
            ...
            .await?;
        let p = proto::GetMaintenanceMarginResponse::decode(resp.as_slice())
...
mod tests {
            ...
            match path {
                "/risk.v1.Query/GetHeatmap" => {
                    Ok(prost::Message::encode_to_vec(&proto::GetHeatmapResponse {
                        market_index: 0,
                        bands_count: 10,
                        mark_price: 50000_00000000,
                        total_at_risk: "1000000".into(),
                    }))
                }
                "/risk.v1.Query/GetOiRatio" => {
                    Ok(prost::Message::encode_to_vec(&proto::GetOiRatioResponse {
                        long_oi: "5000000".into(),
                        short_oi: "3000000".into(),
                    }))
                }
                "/risk.v1.Query/GetMaintenanceMargin" => Ok(prost::Message::encode_to_vec(
                    &proto::GetMaintenanceMarginResponse {
                        margin: "50000".into(),
                    },
                )),
    ...
    async fn get_maintenance_margin_works() {
        let m = make_client()
            .get_maintenance_margin(requests::GetMaintenanceMarginRequest::new(
                0, "100000", 50000, true, 10, 51000,
            ))
            .await
            .unwrap();
        assert_eq!(m, "50000");
