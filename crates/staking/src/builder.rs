...
impl AccrueFeesBuilder {
    ...
    pub fn volume(mut self, volume: impl Into<String>) -> Self {
        self.volume = Some(volume.into()); self
    }
    pub fn fees(mut self, fees: impl Into<String>) -> Self {
        self.fees = Some(fees.into()); self
    }
    pub fn validator_id(mut self, id: impl Into<String>) -> Self {
        self.validator_id = Some(id.into()); self
    }
...
/// Fluent builder for pooling rewards per shard.
#[derive(Default)]
pub struct PoolRewardsBuilder {
    asset_index: Option<u64>,
    pooled_fees: Option<String>,
    epoch_number: Option<u64>,
    sig: Option<Vec<u8>>,
}

impl PoolRewardsBuilder {
    pub fn new() -> Self { Self::default() }

    pub fn asset_index(mut self, idx: u64) -> Self {
        self.asset_index = Some(idx); self
    }
    pub fn pooled_fees(mut self, fees: impl Into<String>) -> Self {
        self.pooled_fees = Some(fees.into()); self
    }
    pub fn epoch_number(mut self, epoch: u64) -> Self {
        self.epoch_number = Some(epoch); self
    }
    pub fn sig(mut self, sig: Vec<u8>) -> Self {
        self.sig = Some(sig); self
    }

    pub fn build(self) -> Result<PoolRewardsRequest, SdkError> {
        let asset_index = self.asset_index.ok_or_else(|| SdkError::invalid_input("asset_index is required for pool rewards"))?;
        let pooled_fees = self.pooled_fees.ok_or_else(|| SdkError::invalid_input("pooled_fees is required for pool rewards"))?;
        let epoch_number = self.epoch_number.ok_or_else(|| SdkError::invalid_input("epoch_number is required for pool rewards"))?;
        let sig = self.sig.ok_or_else(|| SdkError::invalid_input("sig is required for pool rewards"))?;

        Ok(PoolRewardsRequest::new(asset_index, pooled_fees, epoch_number, sig))
    }
}

// ============================================================================
// DistributeRewardsBuilder
// ============================================================================

/// Fluent builder for distributing rewards at epoch end.
#[derive(Default)]
pub struct DistributeRewardsBuilder {
    epoch_number: Option<u64>,
    asset_index: Option<u64>,
    total_rewards: Option<String>,
    validator_stakes: BTreeMap<String, String>,
    vrf_seed: Option<Vec<u8>>,
    sig: Option<Vec<u8>>,
}
...
impl DistributeRewardsBuilder {
    ...
    pub fn total_rewards(mut self, rewards: impl Into<String>) -> Self {
        self.total_rewards = Some(rewards.into()); self
    }
    pub fn validator_stake(mut self, validator_id: impl Into<String>, stake: impl Into<String>) -> Self {
        self.validator_stakes.insert(validator_id.into(), stake.into()); self
    }
    pub fn vrf_seed(mut self, seed: Vec<u8>) -> Self {
        self.vrf_seed = Some(seed); self
    }
    pub fn sig(mut self, sig: Vec<u8>) -> Self {
        self.sig = Some(sig); self
    }

    pub fn build(self) -> Result<DistributeRewardsRequest, SdkError> {
        let epoch_number = self.epoch_number.ok_or_else(|| SdkError::invalid_input("epoch_number is required for reward distribution"))?;
        let asset_index = self.asset_index.ok_or_else(|| SdkError::invalid_input("asset_index is required for reward distribution"))?;
        let total_rewards = self.total_rewards.ok_or_else(|| SdkError::invalid_input("total_rewards is required for reward distribution"))?;
        let sig = self.sig.ok_or_else(|| SdkError::invalid_input("sig is required for reward distribution"))?;

        let mut req = DistributeRewardsRequest::new(epoch_number, asset_index, total_rewards, sig);
        req.validator_stakes = self.validator_stakes;
        if let Some(seed) = self.vrf_seed { req.vrf_seed = seed; }
        Ok(req)
    }
}
...
mod tests {
    ...
    #[test]
    fn distribute_rewards_builder_works() {
        let req = DistributeRewardsBuilder::new()
            .epoch_number(42)
            .asset_index(0)
            .total_rewards("100000")
            .validator_stake("val-1", "50000")
            .validator_stake("val-2", "50000")
            .vrf_seed(vec![1, 2, 3])
            .sig(vec![4, 5])
            .build()
            .unwrap();
        assert_eq!(req.validator_stakes.len(), 2);
    }
    ...
    fn accrue_fees_builder_works() {
        let req = AccrueFeesBuilder::new()
            ...
            .build()
            .unwrap();
        assert_eq!(req.tps, "1000");
    }

    #[test]
    fn pool_rewards_builder_works() {
        let req = PoolRewardsBuilder::new()
            .asset_index(0)
            .pooled_fees("5000")
            .epoch_number(42)
            .sig(vec![1, 2])
