...
/// Request to accrue transaction fees for validator rewards.
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
...
impl AccrueFeesRequest {
    ...
    pub fn to_any(&self) -> ProtoAny {
        let msg: proto::MsgAccrueFeesRequest = self.clone().into();
        ProtoAny {
            type_url: "/staking.v1.MsgAccrueFeesRequest".into(),
            value: msg.encode_to_vec(),
        }
    }
}
...
/// Request to pool rewards per shard for an epoch.
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct PoolRewardsRequest {
    pub asset_index: u64,
    pub pooled_fees: String,
    pub epoch_number: u64,
    pub sig: Vec<u8>,
}

impl PoolRewardsRequest {
    pub fn new(
        asset_index: u64,
        pooled_fees: impl Into<String>,
        epoch_number: u64,
        sig: Vec<u8>,
    ) -> Self {
        Self {
            asset_index,
            pooled_fees: pooled_fees.into(),
            epoch_number,
            sig,
        }
    }
    ...
    pub fn to_any(&self) -> ProtoAny {
        let msg: proto::MsgPoolRewardsRequest = self.clone().into();
        ProtoAny {
            type_url: "/staking.v1.MsgPoolRewardsRequest".into(),
            value: msg.encode_to_vec(),
        }
    }
}

impl From<PoolRewardsRequest> for proto::MsgPoolRewardsRequest {
    fn from(req: PoolRewardsRequest) -> Self {
        Self {
            asset_index: req.asset_index,
            pooled_fees: req.pooled_fees,
...
/// Request to distribute rewards at epoch end.
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct DistributeRewardsRequest {
    ...
    pub validator_stakes: BTreeMap<String, String>,
    pub vrf_seed: Vec<u8>,
    pub sig: Vec<u8>,
}

impl DistributeRewardsRequest {
    pub fn new(
        epoch_number: u64,
        asset_index: u64,
        total_rewards: impl Into<String>,
        sig: Vec<u8>,
    ) -> Self {
        Self {
            epoch_number,
    ...
    pub fn to_any(&self) -> ProtoAny {
        let msg: proto::MsgDistributeRewardsRequest = self.clone().into();
        ProtoAny {
            type_url: "/staking.v1.MsgDistributeRewardsRequest".into(),
            value: msg.encode_to_vec(),
        }
    }
}

impl From<DistributeRewardsRequest> for proto::MsgDistributeRewardsRequest {
    fn from(req: DistributeRewardsRequest) -> Self {
        Self {
            epoch_number: req.epoch_number,
            asset_index: req.asset_index,
            total_rewards: req.total_rewards,
            validator_stakes: req.validator_stakes.into_iter().collect(),
            vrf_seed: req.vrf_seed,
            sig: req.sig,
            timestamp: None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn pool_rewards_request_to_any() {
        let req = PoolRewardsRequest::new(0, "5000", 42, vec![1, 2]);
        let any = req.to_any();
        assert_eq!(any.type_url, "/staking.v1.MsgPoolRewardsRequest");
    }

    #[test]
    fn distribute_rewards_request_to_any() {
        let req = DistributeRewardsRequest::new(42, 0, "10000", vec![3, 4]);
        let any = req.to_any();
        assert_eq!(any.type_url, "/staking.v1.MsgDistributeRewardsRequest");
    }
