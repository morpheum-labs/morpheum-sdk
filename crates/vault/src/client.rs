...
impl VaultClient {
    ...
    pub async fn get_vault(&self, req: requests::GetVaultRequest) -> Result<Vault, SdkError> {
        ...
        let resp = self
            .query(svc_path!("GetVault"), proto_req.encode_to_vec())
            .await?;
        let p = proto::GetVaultResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.vault
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("vault field missing"))
    }

    /// Lists vaults with optional filters.
    pub async fn list_vaults(
        &self,
        req: requests::ListVaultsRequest,
    ) -> Result<VaultListPage, SdkError> {
        let proto_req: proto::ListVaultsRequest = req.into();
        let resp = self
            .query(svc_path!("ListVaults"), proto_req.encode_to_vec())
            .await?;
        let p = proto::ListVaultsResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        Ok(VaultListPage {
            vaults: p.vaults.into_iter().map(Into::into).collect(),
        })
    }

    /// Gets all vaults by agent.
    pub async fn get_vaults_by_agent(
        &self,
        req: requests::GetVaultsByAgentRequest,
    ) -> Result<VaultListPage, SdkError> {
        let proto_req: proto::GetVaultsByAgentRequest = req.into();
        let resp = self
            .query(svc_path!("GetVaultsByAgent"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetVaultsByAgentResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        Ok(VaultListPage {
            vaults: p.vaults.into_iter().map(Into::into).collect(),
        })
    }
    ...
    /// Gets vaults by type.
    pub async fn get_vaults_by_type(
        &self,
        req: requests::GetVaultsByTypeRequest,
    ) -> Result<VaultListPage, SdkError> {
        let proto_req: proto::GetVaultsByTypeRequest = req.into();
        let resp = self
            .query(svc_path!("GetVaultsByType"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetVaultsByTypeResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        Ok(VaultListPage {
            vaults: p.vaults.into_iter().map(Into::into).collect(),
        })
    }
    ...
    pub async fn get_user_stake(
        &self,
        req: requests::GetUserStakeRequest,
    ) -> Result<Stake, SdkError> {
        let proto_req: proto::GetUserStakeRequest = req.into();
        let resp = self
            .query(svc_path!("GetUserStake"), proto_req.encode_to_vec())
            .await?;
        let p = proto::GetUserStakeResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.stake
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("stake field missing"))
    }

    /// Lists all stakes for a user across vaults.
    pub async fn list_user_stakes(
        &self,
        req: requests::ListUserStakesRequest,
    ) -> Result<StakeListPage, SdkError> {
        let proto_req: proto::ListUserStakesRequest = req.into();
        let resp = self
            .query(svc_path!("ListUserStakes"), proto_req.encode_to_vec())
            .await?;
        let p = proto::ListUserStakesResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        Ok(StakeListPage {
            stakes: p.stakes.into_iter().map(Into::into).collect(),
        })
    }
    ...
    pub async fn get_strategy_history(
        &self,
        req: requests::GetStrategyHistoryRequest,
    ) -> Result<StrategyHistoryPage, SdkError> {
        let proto_req: proto::GetStrategyHistoryRequest = req.into();
        let resp = self
            .query(svc_path!("GetStrategyHistory"), proto_req.encode_to_vec())
            .await?;
        let p =
            proto::GetStrategyHistoryResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        Ok(StrategyHistoryPage {
            executions: p.executions.into_iter().map(Into::into).collect(),
        })
    }

    /// Gets IL metrics for a vault.
    pub async fn get_il_metrics(
        &self,
        req: requests::GetIlMetricsRequest,
    ) -> Result<IlMetrics, SdkError> {
        let proto_req: proto::GetIlMetricsRequest = req.into();
        let resp = self
            .query(svc_path!("GetILMetrics"), proto_req.encode_to_vec())
            .await?;
        let p = proto::GetIlMetricsResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.metrics
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("metrics field missing"))
    }

    /// Gets real-time vault health.
    pub async fn get_vault_health(
        &self,
        req: requests::GetVaultHealthRequest,
    ) -> Result<VaultHealth, SdkError> {
        let proto_req: proto::GetVaultHealthRequest = req.into();
        let resp = self
            .query(svc_path!("GetVaultHealth"), proto_req.encode_to_vec())
            .await?;
        let p = proto::GetVaultHealthResponse::decode(resp.as_slice()).map_err(SdkError::Decode)?;
        p.health
            .map(Into::into)
            .ok_or_else(|| SdkError::transport("health field missing"))
    }

    /// Gets top vaults ranked by metric.
    pub async fn get_top_vaults(
        &self,
        req: requests::GetTopVaultsRequest,
    ) -> Result<VaultListPage, SdkError> {
        let proto_req: proto::GetTopVaultsRequest = req.into();
        let resp = self
            .query(svc_path!("GetTopVaults"), proto_req.encode_to_vec())
            .await?;
    ...
    pub async fn get_params(&self) -> Result<VaultParams, SdkError> {
        Err(SdkError::transport(
            "vault params query not exposed in proto Query service — use governance queries",
        ))
    }
}
...
mod tests {
    ...
    impl Transport for DummyTransport {
        ...
        async fn query(&self, path: &str, _: Vec<u8>) -> Result<Vec<u8>, SdkError> {
            match path {
                ...
                "/vault.v1.Query/GetUserStake" => Ok(prost::Message::encode_to_vec(
                    &proto::GetUserStakeResponse {
                        success: true,
                        stake: Some(proto::Stake {
                            stake_id: "s1".into(),
                            address: "morph1user".into(),
                            vault_id: "v1".into(),
                            asset: None,
                            amount: "500".into(),
                            shares: "500".into(),
                ...
                "/vault.v1.Query/GetVaultHealth" => Ok(prost::Message::encode_to_vec(
                    &proto::GetVaultHealthResponse {
                        success: true,
                ...
                "/vault.v1.Query/GetTopVaults" => Ok(prost::Message::encode_to_vec(
                    &proto::GetTopVaultsResponse {
                        success: true,
                        vaults: vec![],
                        pagination: None,
                    },
    ...
    async fn get_vault_works() {
        let v = make_client()
            .get_vault(requests::GetVaultRequest::new("v1"))
            .await
            .unwrap();
    ...
    async fn list_vaults_works() {
        let page = make_client()
            .list_vaults(requests::ListVaultsRequest::new())
            .await
            .unwrap();
